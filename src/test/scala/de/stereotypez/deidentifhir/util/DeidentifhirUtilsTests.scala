...
import de.stereotypez.deidentifhir.util.DeidentifhirUtils.mergePathHandlers
...
class DeidentifhirUtilsTests extends AnyFunSuite {
  ...
  test("merge path handlers from multiple modules") {
    ...
    val test0 = mergePathHandlers(Seq())
    assert(test0.equals(None))
    val test1 = mergePathHandlers(Seq(handlersFromModule3, handlersFromModule4))
    assert(test1.equals(Some(None)))
    val test2 = mergePathHandlers(Seq(handlersFromModule3, handlersFromModule3))
    assert(test2.equals(None))
    val test3 = mergePathHandlers(Seq(handlersFromModule1, handlersFromModule2))
    assert(test3.equals(Some(Some(Seq(h1,h2,h3,h4)))))
    val test4 = mergePathHandlers(Seq(handlersFromModule1, handlersFromModule3, handlersFromModule4, handlersFromModule2))
